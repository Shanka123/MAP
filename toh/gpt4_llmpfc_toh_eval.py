...
frac_solved_3disks_allruns=[]
frac_solved_4disks_allruns=[]
frac_invalid_moves_3disks_allruns=[]
frac_invalid_moves_4disks_allruns=[]
...
for run_no in range(1,6):
    ...
    for file in os.listdir(os.path.join(args.output_dir,'run{}'.format(run_no))):
        ...
        for i in range(task_index,env_feedback_index):
            ...
            if 'Move' in lines_baseline[i] and 'from' in lines_baseline[i] and 'to' in lines_baseline[i] and (('A' in lines_baseline[i] and 'B' in lines_baseline[i] ) or ('C' in lines_baseline[i] and 'B' in lines_baseline[i] ) or ('A' in lines_baseline[i] and 'C' in lines_baseline[i] )):
                ...
                move_line = lines_baseline[i].replace('.','').strip()
    ...
    frac_solved_3disks_allruns.append(np.mean(np.array(frac_solved_3disks)))
    frac_invalid_moves_3disks_allruns.append(total_invalid_moves_3disks/total_moves_3disks)

    frac_solved_4disks_allruns.append(np.mean(np.array(frac_solved_4disks)))
    frac_invalid_moves_4disks_allruns.append(total_invalid_moves_4disks/total_moves_4disks)


print("For 3 disks>>>>")

print("fraction solved without invalid>>>",np.mean(np.array(frac_solved_3disks_allruns)))
print("fraction invalid moves>>",np.mean(np.array(frac_invalid_moves_3disks_allruns)))

print("For 4 disks>>>>")

print("fraction solved without invalid>>>",np.mean(np.array(frac_solved_4disks_allruns)))
print("fraction invalid moves>>",np.mean(np.array(frac_invalid_moves_4disks_allruns)))
