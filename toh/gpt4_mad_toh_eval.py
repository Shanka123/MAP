...
for file in os.listdir(args.output_dir):
    ...
    agent1_index=0
    for q in range(len(lines_baseline)):
        if "GPT-4 Agent1 Response" in lines_baseline[q]:
            agent1_index=q
            break
    
    agent2_index=0
    for q in range(len(lines_baseline)):
        if "GPT-4 Agent2 Response" in lines_baseline[q]:
            agent2_index=q
            break
...
#             print(task_index, agent1_index, agent2_index)
    for i in range(agent1_index,agent2_index):
        ...
        if  'Move' in lines_baseline[i] and 'from' in lines_baseline[i] and 'to' in lines_baseline[i] and (('A' in lines_baseline[i] and 'B' in lines_baseline[i] ) or ('C' in lines_baseline[i] and 'B' in lines_baseline[i] ) or ('A' in lines_baseline[i] and 'C' in lines_baseline[i] )):
            if ":" in lines_baseline[i]:
                
                move_line = lines_baseline[i].split(':')[1].replace('.','').strip() # for random valid only
            else:
                move_line = lines_baseline[i].split('.')[1].replace('.','').strip() # for random valid only
...
#                             continue
#                     print(move_line)
            moves_decoded_tuples.append((int(move_line.split(" ")[1]),move_line.split(" ")[3],move_line.split(" ")[5][0]))
            count = i
#                     print(lines_baseline,count,move_line)
            while ("A =" not in lines_baseline[count]):
                count+=1
                if count>=len(lines_baseline):
                    break
    ...
    if num_moves ==0 or len(all_configuration_after_moves)==1:
        all_solved.append(solved_without_invalid)
        
        # print("skipping file>>",file_no)
        continue
...
#             print(task_index, agent1_index, agent2_index)
    for i in range(agent2_index,len(lines_baseline)):
