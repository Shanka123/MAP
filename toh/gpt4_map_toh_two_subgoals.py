...
import argparse
from azure.identity import ManagedIdentityCredential, DefaultAzureCredential, ChainedTokenCredential, AzureCliCredential, get_bearer_token_provider
import os
...
credential = get_bearer_token_provider(ManagedIdentityCredential(),scope)
...
def task_coordination_module(predicted_state, goal_configuration):
	...
	You will be given a current configuration and a goal configuration, and your task is to say whether the goal configuration matches the current configuration.
...
def subgoal_module(state_A,state_B,state_C,goal_config,num_disks):
	...
	Goal: The goal is to generate two subgoals from the current configuration, that helps in reaching the goal configuration using minimum number of moves.
	
	To generate subgoals use the goal recursion strategy. First if the smallest number isn't at the correct position in list C, then set the subgoal of moving the smallest number to its correct position in list C.
	...
	Note in the subgoal configurations all numbers should always be in ascending order in all the three lists.
	...
	I need to move 0 from list A to list C. 
	Step 1. Find the numbers to the right of 0 in list A. There are none.
	Step 2. Find the numbers larger than 0 in list C. There are none.
	There are no numbers found in Step 1 and Step 2. Hence I will move 0 from list A to list C. Also numbers should be in ascending order in list C.
	Subgoal 2:
	A = []
	B = [1, 2]
	C = [0]
	...
	Step 1. Find the numbers to the right of 0 in list B. There are none.
	Step 2. Find the numbers larger than 0 in list C. There are none.
	There are no numbers found in Step 1 and Step 2. Hence I will move 0 from list B to list C. Also numbers should be in ascending order in list C.
	Subgoal 2:
	A = [1, 2]
	B = []
	C = [0]  
	...
	while cur_try<10:
		...
		try:
			...
			subgoal_response = client.chat.completions.create(model=deployment_name, messages=subgoal_input,temperature=0.0, top_p= 0 ,max_tokens=1000)
	...
	if len(subgoal)==0:
		...
		all_subgoals.append([subgoal[0],subgoal[1],subgoal[2]])
		all_subgoals.append([subgoal[3],subgoal[4],subgoal[5]])
...
parser.add_argument
parser.add_argument('--run_no', type = int, help='run number', required= True)
...
# num_output_tokens = 0
icl_examples = [ 3, 22]
for run_no in range(args.run_no,args.run_no+1):
	...
	for i in range(26):
		if (i+1) not in icl_examples:
			...
			subgoal_configurations = subgoal_module(A, B, C,final_target_configuration,num_disks)
			print("subgoals generated>>>",subgoal_configurations)
			goal_config = subgoal_configurations[0]
			...
			""".format(standard_prompt,"A = "+str(A),"B = "+str(B),"C = "+str(C),"A = "+str(subgoal_configurations[0][0]),"B = "+str(subgoal_configurations[0][1]),"C = "+str(subgoal_configurations[0][2]))
			...
			for step in range(num_steps):
				...
				elif task_coordination_module(next_state_prediction,goal_config):
					print("subgoal reached>>>>")
					# subgoal_flag=1
					if len(subgoal_configurations)==1 or first_subgoal_flag==1:

						goal_config = final_target_configuration
					else:
						first_subgoal_flag = 1
						goal_config = subgoal_configurations[1]
