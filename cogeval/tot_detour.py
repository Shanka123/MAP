...
def extract_moves_configs(response):
	...
	for idx,ind in enumerate(possible_step_start_indices):
		if idx< len(possible_step_start_indices)-1:
			if len(response[ind:len(response)].split(":"))>2:
				all_moves.append(response[ind:possible_step_start_indices[idx+1]].split(":")[1].strip().split("\n")[0].split(",")[0])
			...
			if len(response[ind:len(response)].split(":"))>2:
				...
				all_moves.append(response[ind:len(response)].split(":")[1].strip().split("\n")[0].split(",")[0])
			else:
