...
Goal: The goal is to evaluate the minimum number of steps from the current room to the room with a chest with the highest reward.
...
Your output should end with the the format:
Answer = <N>
'''
...
Goal: The goal is to give a judgement (sure/impossible) if the sequence of steps eventually lead to the room which has a chest with the highest reward, and for each of the steps the rooms are directly connected to each other.
...
Here is the sequence of steps from the starting room to the room with a chest with the highest reward:
...
def value_outputs_unwrap(value_outputs: list,last_step_flag) -> float:
	...
	for output in value_outputs:
		if not last_step_flag:
			v+=int(output.split("Answer =")[-1])

			# first_index = 0
			# for k in range(len(output) - 26):
			# 	if output[k:k+27] == 'that yields the most reward':
			# 		first_index = k
			...
			# 	err = f"Error: {str(e)}"
			# 	print(err)
			# 	v+=int(output[first_index:second_index].split("(")[0].strip().split(" ")[-1])
