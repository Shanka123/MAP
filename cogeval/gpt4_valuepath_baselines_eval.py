...
#             task_index = i
    ...
    for i in range(len(lines_baseline)):
        if "GPT-4 Response" in lines_baseline[i]:
            ...
            rooms_splits = lines_baseline[count].split(":")[1].strip().split(",")
...
#             print("Invalid move because room {} is not connected to room {}.".format(int(rooms_splits[k]),int(rooms_splits[k+1])))
    ...
    if int(rooms_splits[num_moves]) in reward_rooms and num_invalid_moves==0:
        ...
        if int(rooms_splits[num_moves])==8:
            optimal_steps.append((valuepath_optimal_steps[file_no-1],num_moves))
...
#     print("fraction invalid moves>>>",num_invalid_moves/num_moves)
#     print("total reward>>",total_reward)
#     print("solved without invalid>>",solved_without_invalid)