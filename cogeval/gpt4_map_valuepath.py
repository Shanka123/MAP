...
def task_coordination_module(current_room):
...
def actor_module_propose_two_actions(actor_input,current_room):
...
def rollout_from_2nodes(current_room1,current_room2,start_prompt,proposals):
	if task_coordination_module(current_room1) or task_coordination_module(current_room2):

		moves_from_current_room1 = state_evaluator_module(current_room1)
		moves_from_current_room2 = state_evaluator_module(current_room2)
		...
		next_move_proposals1 = actor_module_propose_two_actions(input,current_room1)
		...
		next_move_proposals2 = actor_module_propose_two_actions(input,current_room2)
		...
		moves_from_current_room1_1 = state_evaluator_module(current_room1_1)
		moves_from_current_room1_2 = state_evaluator_module(current_room1_2)
		moves_from_current_room2_1 = state_evaluator_module(current_room2_1)
		moves_from_current_room2_2 = state_evaluator_module(current_room2_2)
		...
		if start_room!=8 and start_room!=15 and start_room!=1 and start_room!=6:
			...
			for step in range(10):
				...
				if step ==0 or next_states is None:
					proposals = actor_module_propose_two_actions(input,current_room)
					...
					next_move_chosen_for_system, next_states, next_next_moves = rollout_from_2nodes(current_room1,current_room2,root_prompt,proposals)

				else:
					next_move_chosen_for_system, next_states, next_next_moves = rollout_from_2nodes(next_states[0],next_states[1],root_prompt,next_next_moves)
				...
				if task_coordination_module(current_room):
					flag=1
					test_dir = './logs/'
					check_path(test_dir)
