...
Goal: The goal is to predict the minimum number of steps from the starting room to reach the target room.
...
To reach the target room (room 4) from the starting room (room 9), the minimum number of steps is 2.
...
To reach the target room (room 10) from the starting room (room 2), the minimum number of steps is 3.
...
To reach the target room (room 3) from the starting room (room 7), the minimum number of steps is 4.
...
Goal: The goal is to give a judgement (sure/impossible) if the sequence of steps from the starting room reach the target room. Check if the steps eventually lead to the the target room, and for each of the steps the rooms are directly connected to each other.
...
Go from room 11 to room 2.

Go from room 2 to room 3.

Go from room 3 to room 6.
...
def value_outputs_unwrap(value_outputs: list,last_step_flag) -> float:
	...
	for output in value_outputs:
		if not last_step_flag:
			...
			first_index=0
			for k in range(len(output) - 26):
			    if output[k:k+27] == 'The minimum number of steps' or output[k:k+27] == 'the minimum number of steps':
			        first_index = k
...
def get_value( y, target_room,n_evaluate_sample,value_cache):
	...
	if y[-1] == target_room:
		...
		prompt = value_last_step_prompt.format(r1=str(y[0]),r2=str(target_room),step_seq=step_seq)
		
		last_step_flag = True
	else:

		prompt = value_prompt.format(r1 = str(y[-1]),r2=str(target_room) )
...
def get_values(ys,target_room, n_evaluate_sample,value_cache):
	values = []

	for y in ys:  # each partial output
	 

		value,value_cache = get_value( y,target_room, n_evaluate_sample,value_cache)
	...
	for start_target_tup in all_step_paths:
		...
		for count in range(2):
			...
			if (start_room,final_target_room) not in icl_examples:
				...
				ys = [[start_room]]
				potential_solutions = []
				print("start and target room>>",start_room,final_target_room)
				for step in range(6):
					...
					values,value_cache = get_values(new_ys,final_target_room, 3,value_cache)
					...
					for select_id in select_ids:
						if new_ys[select_id][-1] == final_target_room:
							print("target room reached>>>")
				...
				with open(output_dir+'problem{}_{}_optimal_steps_{}_reward_{}.pkl'.format(start_room,final_target_room,optimal_num_steps,optimal_reward), 'wb') as f:
