...
def task_coordination_module(current_room,reval_flag):
...
def state_evaluator_module(current_room,reval_flag):
...
def actor_module_propose_two_actions(actor_input,current_room,reval_flag):
	...
	while num_tries<10:
		...
		if len(actor_truncated_response)==2:
			...
			move_validity1, validity1_response = move_validator_module(int(actor_truncated_response[0].split(" ")[3]), int(actor_truncated_response[0].split(" ")[6]))
			move_validity2, validity2_response = move_validator_module(int(actor_truncated_response[1].split(" ")[3]), int(actor_truncated_response[1].split(" ")[6]))
...
def rollout_from_2nodes(current_room1,current_room2,start_prompt,proposals,reval_flag):
	if task_coordination_module(current_room1,reval_flag) or task_coordination_module(current_room2,reval_flag):

		moves_from_current_room1 = state_evaluator_module(current_room1,reval_flag)
		moves_from_current_room2 = state_evaluator_module(current_room2,reval_flag)
		...
		next_move_proposals1 = actor_module_propose_two_actions(input,current_room1,reval_flag)
		...
		next_move_proposals2 = actor_module_propose_two_actions(input,current_room2,reval_flag)
		...
		moves_from_current_room1_1 = state_evaluator_module(current_room1_1,reval_flag)
		moves_from_current_room1_2 = state_evaluator_module(current_room1_2,reval_flag)
		moves_from_current_room2_1 = state_evaluator_module(current_room2_1,reval_flag)
		moves_from_current_room2_2 = state_evaluator_module(current_room2_2,reval_flag)
		...
		if start_room!=8 and start_room!=15:
			...
			while step < 10:
				...
				if step ==0 or next_states is None:
					proposals = actor_module_propose_two_actions(input,current_room,reval_flag)
					...
					next_move_chosen_for_system, next_states, next_next_moves = rollout_from_2nodes(current_room1,current_room2,root_prompt,proposals,reval_flag)

				else:
					next_move_chosen_for_system, next_states, next_next_moves = rollout_from_2nodes(next_states[0],next_states[1],root_prompt,next_next_moves,reval_flag)
				...
				step+=1
				if task_coordination_module(current_room,reval_flag):
			...
			print("List of rooms visited before rewardReval = {}".format(rooms_visited_before))
			print("List of rooms visited after rewardReval = {}".format(rooms_visited_after))
